...
ifeq ($(DEBUG),1)
	LIB_DIR = $(BUILD_DIR)/tmp/wasm-standalone-cli/debug/lib
else
	ifeq ($(INSTRUMENT),1)
		LIB_DIR = $(BUILD_DIR)/tmp/wasm-standalone-cli/instrument/lib
	else
		LIB_DIR = $(BUILD_DIR)/tmp/wasm-standalone-cli/release/lib
	endif
endif
...
all: $(LIB_DIR) $(BIN_DIR)/$(NAME).js

$(LIB_DIR):
	mkdir -p $(LIB_DIR)

$(TMP_DIR)/wasm_standalone_cli.c: $(XS_DIR)/platforms/wasm_standalone_cli.c
	cp $^ $@

$(TMP_DIR)/wasm_standalone_cli.o: $(TMP_DIR)/wasm_standalone_cli.c $(TMP_DIR)/mc.xs.h $(XS_HEADERS)
	@echo "# copy" $(<F)
	$(CC) $(C_DEFINES) $(C_INCLUDES) $(C_FLAGS) $< -o $@

$(BIN_DIR)/$(NAME).js: $(TMP_DIR)/wasm_standalone_cli.o $(XS_OBJECTS) $(TMP_DIR)/mc.xs.c.o $(TMP_DIR)/mc.resources.c.o $(OBJECTS)
	@echo "# cc" $(@F)
