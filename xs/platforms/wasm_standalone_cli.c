	...
	xsBeginHost(machine);
	{
		xsVars(3);
		{
			xsTry {
				...
				printf("wasm_standalone_cli: loading top-level main.js\n");
				xsVar(1) = xsCall1(xsGlobal, xsID_require, xsString("main"));
				printf(" wasm_standalone_cli: loaded\n");

				printf("wasm_standalone_cli: invoking main(argv)\n");
				xsCallFunction1(xsVar(1), xsUndefined, xsVar(0));
				printf(" wasm_standalone_cli: invoked\n");
			}
			xsCatch {
